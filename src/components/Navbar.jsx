		...
		<nav className="navbar navbar-dark bg-dark mb-3 px-5 border-bottom border-secondary d-flex align-items-center">
			...
			<div className="ml-auto">
				...
				<div className="dropdown">
					<button
						className="btn btn-outline-warning shadow-lg d-flex align-items-center fw-bold border-2" 
						type="button"
					...
					<ul className="dropdown-menu dropdown-menu-end bg-secondary" aria-labelledby="favoritesDropdown" style={{ minWidth: "220px" }}>
						{store.favoritos.length === 0 ? (
							<li className="dropdown-item text-center text-light small italic">Empty list</li>
						) : (
							store.favoritos.map((fav, index) => (
								<li key={index} className="dropdown-item d-flex justify-content-between align-items-center text-white">
									<Link to={`/detalle/${fav.id}`} className="text-decoration-none text-white flex-grow-1">
										{fav.name}
									</Link>

									{/* Botón para eliminar del array de favoritos */}
									<i
										className="fa-solid fa-trash-can text-danger ms-2"
										style={{ cursor: "pointer" }}
										onClick={(e) => {
											e.stopPropagation(); // Evita que el menú se cierre al borrar
											dispatch({ type: "delete_favorito", payload: fav.id });
										}}
									></i>
								</li>
							))
